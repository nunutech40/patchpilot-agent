...
```json
{
  ...
  "coding_guidance": {
    ...
    "likely_actions": [
      ...
      "Check Android Gradle Plugin and Gradle wrapper compatibility before changing versions."
    ],
    "do_not_change": [
      "Do not modify app business logic.",
      "Do not remove permissions unless the policy explicitly requires it."
    ]
  },
...
When the Runtime MR Agent queries Elastic during `/check`, it should transform
matching policy records into a coding-task payload for Antigravity. The Policy
Context Agent has already defined the generic requirement and coding guidance;
the Runtime MR Agent adds repository facts and creates the concrete task.
...
- `policy_context.recommended_action`: generic action generated by the Policy
  Context Agent from official docs.
- `policy_context.coding_guidance`: bounded coding hints and explicit
